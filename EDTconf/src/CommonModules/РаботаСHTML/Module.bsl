...
Процедура ПередатьОбъектыСтраницыНаКлиента(ТекстСтраницы,ТаблицаОбъектовСтраницы)  экспорт 
	//Внимание! Увеличивает обьем трафика т.к. фактически выкачивает картинки на клиента

	#Если ВебКлиент Тогда
		Возврат; //закоментировать в случае необходимости сохранения файлов на клиента
		//Внимание! Требует установки расширения работы с файлами!
	#КонецЕсли
	
	//поставим ограничение - выкачиваем объекты на клиента только если у нас тонкий клиент веб сервер
	//В противном случае объекты будут во временном хранилище
	Если Найти(НРЕГ(СтрокаСоединенияИнформационнойБазы()),"ws=") = 0 Тогда 
		//Возврат;   //Пока что закоментировал т.к. при работе в тонком клиенте с включенной усилиенной безопасностью или заблокированным IE картинки не отображаются
	КонецЕсли;	
